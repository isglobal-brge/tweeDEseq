...
<<>>=
data(genderGenesNigerian)
data(hkGenesNigerian)

length(XiEgenes)
length(msYgenes)
length(hkGenesNigerian)
@
...
<<>>=
set.seed(123)
someHKgenes <- sample(hkGenesNigerian, size=25)
...
@
and we can examine the result by means of the quantile-quantile plots produced with the function \Rfunction{qqchisq()} and shown in Figure~\ref{fig:gof}, which indicates that about 50\% of the genes show a substantial discrepancy with the respect to the negative binomial distribution.

<<gof, fig=TRUE, include=FALSE, echo=TRUE, results=verbatim, height=7, width=12>>=
par(mfrow=c(1,2), mar=c(4, 5, 3, 4))
qq <- qqchisq(chi2gof, main="Chi2 Q-Q Plot")
qq <- qqchisq(chi2gof, normal=TRUE)
@
\begin{figure}[ht]
\begin{center}
\begin{tabular}{c}
...
\end{tabular}
\end{center}
\caption{Goodness of fit to a binomial distribution. On the left a quantile-quantile plot of the $\chi^2$ statistic employed to assess the goodness-of-fit of the RNA-seq data to a negative binomial distribution is shown. About 50\% of the genes have expression profiles that depart substantially from the negative binomial distribution. On the right we have the same data but $\chi^2$ statistics are transformed into standard normal z-scores to improve visibility of the lower quantiles.}
\label{fig:gof}
\end{figure}
...
data(annotNigerian)
head(annotNigerian)
deGenes <- merge(deGenes, annotNigerian, by="row.names", sort=FALSE)
@
